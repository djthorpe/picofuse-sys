...
typedef enum {
  ...
  hid_state_click = (1 << 2),        ///< A single click event was generated.
  hid_state_double_click = (1 << 3), ///< A double-click event was generated.
  ...
  hid_state_scroll_lock = (1 << 10), ///< Scroll Lock state is active.
  hid_state_left_shift = (1 << 11),  ///< Left Shift modifier is active.
  hid_state_right_shift = (1 << 12), ///< Right Shift modifier is active.
  hid_state_left_meta = (1 << 13), ///< Left Meta (Command) modifier is active.
  hid_state_right_meta =
      (1 << 14),                   ///< Right Meta (Command) modifier is active.
  hid_state_left_alt = (1 << 15),  ///< Left Option/Alt modifier is active.
  hid_state_right_alt = (1 << 16), ///< Right Option/Alt modifier is active.
  hid_state_left_control = (1 << 17),  ///< Left Control modifier is active.
  hid_state_right_control = (1 << 18), ///< Right Control modifier is active.
  hid_state_left_windows = (1 << 19), ///< Left Windows/Meta modifier is active.
  hid_state_right_windows =
      (1 << 20), ///< Right Windows/Meta modifier is active.
  hid_state_shift = (hid_state_left_shift |
                     hid_state_right_shift), ///< Convenience mask: either left
                                             ///< or right Shift.
  hid_state_meta = (hid_state_left_meta |
                    hid_state_right_meta), ///< Convenience mask: either left
                                           ///< or right Meta (Command).
  hid_state_alt = (hid_state_left_alt |
                   hid_state_right_alt), ///< Convenience mask: either left or
                                         ///< right Alt (Option).
  hid_state_control = (hid_state_left_control |
                       hid_state_right_control), ///< Convenience mask: either
                                                 ///< left or right Control.
  hid_state_windows =
      (hid_state_left_windows |
       hid_state_right_windows), ///< Convenience mask: either left or right
                                 ///< Windows/Meta.
} hid_state_t;
...
static inline hid_state_t keycode_to_state(uint16_t keycode) {
  ...
  case KEYCODE_RIGHTCTRL:
    return hid_state_right_control;
  case KEYCODE_LEFTSHIFT:
    return hid_state_left_shift;
  case KEYCODE_RIGHTSHIFT:
    return hid_state_right_shift;
  case KEYCODE_LEFTALT:
    return hid_state_left_alt;
  case KEYCODE_RIGHTALT:
    return hid_state_right_alt;
  case KEYCODE_LEFTMETA:
    return hid_state_left_meta;
  case KEYCODE_RIGHTMETA:
    return hid_state_right_meta;
  case KEYCODE_CAPSLOCK:
    return hid_state_caps_lock;
  case KEYCODE_NUMLOCK:
    return hid_state_num_lock;
  case KEYCODE_SCROLLLOCK:
    return hid_state_scroll_lock;
  default:
